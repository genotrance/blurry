...
def main(args):
    "Main entry point"
    mainmodname = os.path.basename(__file__)[:-3]
    mainmod = importlib.import_module(mainmodname)

    while True:
        blurry = mainmod.Blurry(args)
        blurry.start()
        if blurry.is_reload is False:
            break

        for module in [helper, sim, image, gui, mainmod]:
